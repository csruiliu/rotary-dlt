...
do
  ...
  do
    ...
    do
      ...
      do
        rm -rf __pycache__
        python3 profile_accuracy.py -p ${PROFILE} -m ${model} -b ${batch} -o ${opt} -r ${lr} -e ${EPOCH}
      done
    done
  done
done
    ...
    do
      ...
      do
        ...
        if [ "${model}" = "resnet" ]; then
          ...
          do
            python3 profile_accuracy.py -p ${PROFILE} -m ${model} -l ${layer} -b ${batch} -o ${opt} -r ${lr} -e ${EPOCH}
          done
          ...
          do
            python3 profile_accuracy.py -p ${PROFILE} -m ${model} -g ${group} -b ${batch} -o ${opt} -r ${lr} -e ${EPOCH}
          done
        elif [ "${model}" = "shufflenetv2" ]; then
          CPLX_LIST="0.5 1 1.5 2"
          for complex in ${CPLX_LIST}
          do
            python3 profile_accuracy.py -p ${PROFILE} -m ${model} -x ${complex} -b ${batch} -o ${opt} -r ${lr} -e ${EPOCH}
          done
