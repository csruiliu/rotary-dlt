...
sem_convergence = mp.Semaphore(cfg_rotary.num_gpu)
sem_others = mp.Semaphore(cfg_rotary.num_gpu)
gpu_slot_convergence = mp.Array('i', [0] * cfg_rotary.num_gpu)
...
def train_job_convergence(shared_runtime_history,
                          shared_accuracy_history):
    sem_convergence.acquire()
    ...
    while True:
        ...
        if gpu_slot_convergence[slot_idx] == 0:
            gpu_device = slot_idx
            gpu_slot_convergence[slot_idx] = 1
            break
            ...
            with tf.Session(config=initialize_config()) as sess:
                ...
                while True:
                    ...
                    if delta <= job_slo_value:
                        ...
                        gpu_slot_convergence[gpu_device] = 0
                        sem_convergence.release()
                        return msg
...
if __name__ == "__main__":
    ...
    proc_pool_convergence = mp.Pool(num_gpu, maxtasksperchild=1)
    ...
    while len(ml_workload_convergence) != 0:
        ...
        accuracy_job_num = len(ml_workload_convergence)
        for idx in range(accuracy_job_num):
            result = proc_pool_convergence.apply_async(train_job_convergence, args=(job_runtime_history,
