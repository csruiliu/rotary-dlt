        ...
        with tf.device(assign_device):
            ...
            with tf.Session(config=config) as sess:
                ...
                for i in range(num_batch):
                    ...
                    if i != 0:
                        end_time_step = timer()
                        step_time_sum += end_time_step - start_time_step

                job_steptime_dict[job_name] = step_time_sum / num_batch
                cur_accuracy = sess.run(eval_op, feed_dict={feature_ph: eval_feature, label_ph: eval_labels})
                end_time = timer()

                job_runtime_dict[job_name] += end_time - start_time
        ...
        if job_data['model'] in ['word2vec']:
            ...
            with tf.device(assign_device):
                ...
                with tf.Session(config=config) as sess:
                    ...
                    job_steptime_dict[job_name] = step_time_sum / num_train_epochs
            ...
            with tf.device(assign_device):
                ...
                with tf.Session(config=config) as sess:
                    ...
                    job_accuracy_dict[job_name] = cur_accuracy
                    job_runtime_dict[job_name] += dur_time
                    job_accuracy_dict[job_name] = cur_accuracy


def train_job(num_gpu):
    job_data = job_queue.get_nowait()
    job_name = str(job_data['id']) + '-' + job_data['model']
...
if __name__ == "__main__":
    ...
    job_runtime_dict = Manager().dict()
    job_steptime_dict = Manager().dict()
    job_deadline_dict = Manager().dict()
    job_done_dict = Manager().dict()

    proc_pool = Pool(processes=n_gpu)

    # plan ahead of running according to deadline
    sched_workload = tetrisched_orchestrate(ml_workload)
    ...
    for job in ml_workload:
        proc_pool.apply_async(func=run_trial, args=(n_gpu,))

    for job in ml_workload:
        proc_pool.apply_async(func=train_job, args=(n_gpu,))
