...
from rotary.sched.sched_common import init_tf_config, init_tf_vars, get_bert_dataset, compared_item
            ...
            try:
                ...
                with tf.device(assign_device):
                    with tf.Session(config=init_tf_config()) as sess:
                        ...
                        self.job_dict_runtime[job_name] += preparation_end_marker - preparation_start_marker
                    ...
                    with tf.Session(config=init_tf_config()) as sess:
                        ...
                        while True:
                            ...
                            for b in range(num_batch):
                                ...
                                sess.run(train_op,
                                         feed_dict={feature_ph: train_data_batch,
                            ...
                            for be in range(num_batch_eval):
                                ...
                                acc_batch = sess.run(eval_op,
                                                     feed_dict={feature_ph: eval_feature_batch,
    ...
    def run(self):
        ...
        while len(self.ml_workload_accuracy) != 0:
            ...
            for idx in range(accuracy_job_num):
                result = self.proc_pool_accuracy.apply_async(self.train_job_accuracy,
                                                             args=(self.job_history_runtime,
        ...
        while not self.job_queue_others.empty():
            ...
            for idx in range(self.num_gpu):
                result = self.proc_pool_others.apply_async(self.train_job_others,
                                                           args=(self.job_history_runtime,
                                                                 self.job_history_accuracy))
