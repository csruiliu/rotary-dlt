...
class LCF:
    ...
    def __init__(self,
        ...
        # init semaphore
        self.sem_convergence = mp.Semaphore(self.num_gpu)
        ...
        self.proc_pool_convergence = mp.Pool(self.num_gpu, maxtasksperchild=1)
        ...
        for job in ml_workload:
            if job['goal_type'] == 'convergence':
                self.ml_workload_convergence.append(job)
            else:
    ...
    def check_job_progress_convergence(self,
                                       job_name,
                                       pre_accuracy,
                                       cur_accuracy,
                                       job_slo_value,
        ...
        if delta <= job_slo_value:
            self.complete_job_convergence(job_name, gpu_device, attain=1)
            return JobStatus.COMPLETE_ATTAIN
        elif self.job_dict_epoch[job_name] >= job_slo_max_time:
            self.complete_job_convergence(job_name, gpu_device, attain=0)
            ...
            with tf.device(assign_device):
                with tf.Session(config=init_tf_config()) as sess:
                    ...
                    while running_slot_time < self.running_slot:
                        ...
                        if mode == SchedType.SCHED_CONVERGENCE:
                            job_progress = self.check_job_progress_convergence(job_name,
                                                                               pre_accuracy,
                                                                               cur_accuracy,
                                                                               job_slo_value,
                                                                               job_slo_max_time,
    ...
    def process_job_convergence(self):
        ...
        while True:
            ...
            if self.gpu_slot_convergence[slot_idx] == 0:
                gpu_device = slot_idx
        ...
        try:
            job_data = self.ml_workload_convergence.pop()
        except IndexError:
            return
        ...
        if job_model == 'bert':
            self.train_bert_model(job_data,
                                  gpu_device,
                                  process_start_marker,
                                  mode=SchedType.SCHED_CONVERGENCE)
    ...
    def run(self):
        print('*********** start processing convergence slo jobs ***********')
        while len(self.ml_workload_convergence) != 0:
            results_convergence = list()
            convergence_job_num = len(self.ml_workload_convergence)
            for idx in range(convergence_job_num):
                result = self.proc_pool_convergence.apply_async(self.process_job_convergence)
